...
if ! [ "`grep ^DNAT /tmp/${remote_ip}_${remote_port}.log |grep ${remote_ip} |grep ${remote_port} |grep ${local_ip}|grep tcp`" ] ; then
 iptables -t nat -I PREROUTING -d $local_ip -p tcp --dport $local_port -j DNAT --to $remote_ip:$remote_port
fi

if ! [ "`grep ^DNAT /tmp/${remote_ip}_${remote_port}.log |grep ${remote_ip} |grep ${remote_port} |grep ${local_ip}|grep udp`" ] ; then
